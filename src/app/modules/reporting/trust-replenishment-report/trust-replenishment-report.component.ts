...
@Component({
  selector: 'app-trust-replenishment-report',
  templateUrl: './trust-replenishment-report.component.html',
  styleUrls: ['./trust-replenishment-report.component.scss'],
  encapsulation: ViewEncapsulation.Emulated,
})
export class TrustReplenishmentReportComponent implements OnInit, OnDestroy {
  ...
  public columnHeaderObj = [
    ...
    {
      name: 'minimumPrimaryRetainerTrustBalance',
      displayName: 'Minimum Primary Retainer Trust Balance',
    },
    { name: 'amountToReplenish', displayName: 'Amount to Replenish' },
  ];
  ...
  submitTrustReplenishmentReport() {
    let data: any = {};
    ...
    this.reportService
      .v1ReportTrustReplenishmentReportPost$Json$Response({ body: data })
      .subscribe(
        (suc: {}) => {
          const res: any = suc;
          this.loading = false;
          ...
          this.ExportToCSV('Trust Replenishment', this.rows, this.columnList);
        },
        () => {
          this.loading = false;
        }
