...
@Component({
  selector: 'app-accounting-echeck-transaction',
  templateUrl: './accounting-echeck-transaction.component.html',
  styleUrls: ['./accounting-echeck-transaction.component.scss'],
  encapsulation: ViewEncapsulation.Emulated
})
export class AccountingEcheckTransactionComponent implements OnInit {
  ...
  public selectedAccount:string = 'All';
  public searchValue = "";
  ...
  loadManualTransferQueueList() {
    ...
    this.trustAccountService
      .v1TrustAccountGetECheckQueueGet$Response({}).subscribe((data: {}) => {
        const res: any = data;
          ...
          if (parsedRes != null && parsedRes.results) {
            if (parsedRes.results.length) {
              ...
              this.oriArr2.forEach(obj => {
                if(obj.targetAccountInfo.title && obj.targetAccountInfo.title.toLowerCase().includes('e-check')) {
  ...
  private matchName(item: any, searchValue: string, fieldName: string): boolean {
    ...
    if (fieldName === 'sourceAccountInfo') {
      ...
      searchSubTitle.search(searchValue.toUpperCase()) > -1;
    }
