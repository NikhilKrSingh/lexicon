...
@Component({
  selector: 'app-new-billing-payment-method',
  templateUrl: './new-payment-method.component.html',
  encapsulation: ViewEncapsulation.Emulated
})
export class NewBillingPaymentMethodComponent
  ...
  @Output() readonly refreshBillingPreference: EventEmitter<any> = new EventEmitter<any>();
  ...
  public ccLoading = false;
  public eCheckLoading = false;
  public autoPayMatters: any = [];
  hasPermissionToAdd: boolean = false;
  @Input() showPayementMethod: boolean = false;
  @Output() readonly showPayementMethodChange: EventEmitter<any> = new EventEmitter<any>();
  ...
  ngOnInit() {
    ...
    if (this.matterDetails) {
      if (this.matterDetails.clientName) {
        this.getClientInfo(this.matterDetails.clientName.id);
        this.getStateList();
      }
      let loginUserAttorny = UtilsHelper.checkPermissionOfRepBingAtn(this.matterDetails);
    ...
    } else if (this.potentialClientDetails) {
      let isConsultAttorney = UtilsHelper.checkPermissionOfConsultAtn(this.potentialClientDetails);
      if (this.permissionList.BILLING_MANAGEMENTisEdit ||
        this.permissionList.BILLING_MANAGEMENTisAdmin ||
        this.permissionList.CLIENT_CONTACT_MANAGEMENTisAdmin || isConsultAttorney) {
        this.hasPermissionToAdd = true;
        this.primaryAddress = this.potentialClientDetails.addresses[0];
        this.clientDetails = this.potentialClientDetails;
        this.getClientInfo(this.potentialClientDetails.id);
  ...
  ngOnChanges(changes) {
    ...
    if (changes.potentialClientDetails && changes.potentialClientDetails.currentValue) {
      this.clientDetails = this.potentialClientDetails;
    }
  }
  ...
  private getClientInfo(clientId) {
    this.clientService
      .v1ClientClientIdGet({
        clientId: clientId
      })
  ...
  private getPaymentMethods() {
    if (this.type == 'matter') {
      ...
      this.getCreditCards(this.potentialClientDetails.id);
      this.getEcheckList(this.potentialClientDetails.id);
    }
  }
  ...
  private addCreditCard() {
    ...
    if (this.potentialClientDetails) {
      component.isPotentialClient = true;
    }
  ...
  private createAddress(
    ...
    const address = {
      ...
      zipCode: res.zipCode,
      personId: this.potentialClientDetails ? this.potentialClientDetails.id : this.matterDetails.clientName.id
    } as vwAddressDetails;
  ...
  private addECheck() {
    ...
    if (this.potentialClientDetails) {
      modalRef.componentInstance.isPotentialClient = true;
    }
  ...
  openCreditCardModal(row) {
    ...
    component.address = this.clientDetails.addresses.find(a => a.id == row.addressId && row.addressId != this.primaryAddress.id) || {};
    component.validateAutoPay = this.validateAutoPay;
    component.paymentComponent = this;
    component.paymentPlanList = this.paymentPlanList;
    if (this.potentialClientDetails) {
  ...
  private validateAutoPay(
    ...
    paymentComponent: NewBillingPaymentMethodComponent
  ) {
    if (payment.autoPay) {
  ...
  displayPaymentMethod() {
    this.showPayementMethod = !this.showPayementMethod;
    this.showPayementMethodChange.emit(this.showPayementMethod);
  }
