...
@Component({
  selector: 'app-add-edit-corp-contacts',
  templateUrl: './add-edit-corp-contacts.component.html',
  styleUrls: ['./add-edit-corp-contacts.component.scss'],
  encapsulation: ViewEncapsulation.Emulated
})
export class AddEditCorpContactsComponent implements OnInit {
  @Input() fromMatter: boolean;
  @Input() fromPotentialClient: boolean;
  @Output() readonly returnToPC = new EventEmitter();
  ...
  public clientId;
  public numberClientId: number;
  public clientDetail;
  public contactErrorMessage: boolean = false;
  constructor(
  ...
  ngOnInit() {
    ...
    } else if (this.fromPotentialClient) {
      this.route.queryParams.subscribe(params => {
        this.clientId = params['clientId'];
        this.numberClientId = Number(this.clientId);
  ...
  public getCorporateContact() {
    ...
    let currentClientId;
    if (this.fromMatter) {
      currentClientId = this.matterDetails.clientName.id;
    } else if (this.fromPotentialClient) {
      currentClientId = this.numberClientId;
    }
  ...
  deleteContact(contact, type) {
    ...
    if (this.fromMatter) {
      ...
      (message =
        'Are you sure you want to remove this Corporate Contact from this potential client?'),
        'Delete',
        'Cancel',
        'Delete Corporate Contact';
    }
    this.dialogService.confirm(message).then(res => {
      if (res) {
        if (type === 'isPrimary') {
          let index = -1;
        ...
        this.corporateContactList.forEach(corporateContact => {
          if (corporateContact.uniqueNumber === contact.uniqueNumber) {
            ...
            corporateContact.isDelete =
              !corporateContact.isPrimary &&
              !corporateContact.isBilling &&
              !corporateContact.generalCounsel;
          }
        });
      }
    });
  ...
  public saveCorporateContact(missingInfoDOM) {
    if (
      this.createType === 'create' &&
      this.emailExistenceForCorporateContact
    ) {
      return;
    }
      ...
      if (!this.editDetails.isEdit) {
        if (this.createType === 'create') {
          ...
          if (this.selectedExistedContactList.length) {
            const existcc = this.generalContactList.find(item => +item.uniqueNumber === +this.selectedExistedContactList[0].uniqueNumber);
            if (existcc) {
              return;
            }
            ...
            this.selectedExistedContactList[0].uniqueNumber = +this
              .selectedExistedContactList[0].uniqueNumber
              ? Number(this.selectedExistedContactList[0].uniqueNumber)
              : 0;
  ...
  navigateToCorporateContactMatter() {
    this.router.navigate(['/matter/dashboard'], {
      queryParams: { selectedtab: 'corporateContact', matterId: this.matterId }
    });
  }

  navigateToCorporateContactPC() {
    this.returnToPC.emit();
  }
  ...
  runConflictCheck() {
    if (!this.primaryContactList.length || !this.billingContactList.length) {
      ...
      if (this.fromMatter) {
        ...
        conflictCheckObject.clientId = this.numberClientId;
        conflictCheckObject.matterId = this.clientDetail.matterId;
        conflictCheckObject.clientCompanyName = this.clientDetail.companyName;
      }
      ...
      this.corporateContactList.forEach(corporateContact => {
        if (!corporateContact.isDelete) {
          conflictCheckObject.corporatecontacts.push({
            ...
            companyName:
              corporateContact.client || corporateContact.companyName,
      ...
      this.corporateContactLoading = true;
      this.contactService
        .v1ContactsCorporateContactConflictCheckPost$Json({
          body: conflictCheckObject
        })
        .pipe(
  ...
  saveData() {
    const corporateContactData = this.corporateContactList.map(
      corporateContact => Object.assign({}, corporateContact)
    );
    this.corporateContactLoading = true;
    corporateContactData.forEach(corporateContact => {
      corporateContact.id = corporateContact.personId
        ? corporateContact.personId
        : 0;
      ...
      corporateContact.isNew =
        corporateContact.isNew || corporateContact.id === 0;
      corporateContact.isDelete = corporateContact.isDelete
        ? corporateContact.isDelete
        : false;
      if (corporateContact.isDelete) {
        corporateContact.isPrimary = corporateContact.originalPrimary;
        corporateContact.isBilling = corporateContact.originalBilling;
        corporateContact.isGeneralCounsel =
          corporateContact.originalGeneralCounsel;
      }
      corporateContact.primaryPhoneNumber = corporateContact.primaryPhone
        ? corporateContact.primaryPhone
        : corporateContact.primaryPhoneNumber;
      delete corporateContact.personId;
      delete corporateContact.generalCounsel;
    });
    let corporateContactsObject;
    if (this.fromMatter) {
      corporateContactsObject = {
        clientId: this.matterDetails.clientName.id,
        corporatecontacts: corporateContactData
      };
    } else if (this.fromPotentialClient) {
      corporateContactsObject = {
        clientId: this.numberClientId,
        corporatecontacts: corporateContactData
      };
    }

    this.contactService
      .v1ContactsUpdateCorporateContactsOfClientPost$Json({
        body: corporateContactsObject
      })
      .subscribe(
        () => {
          this.toastrService.showSuccess(
            'Corporate contacts updated.'
          );
          this.corporateContactLoading = false;
          if (this.fromMatter) {
            this.navigateToCorporateContactMatter();
          } else if (this.fromPotentialClient) {
            this.navigateToCorporateContactPC();
          }
        },
        () => {
          this.corporateContactLoading = false;
        ...
        this.emailExistenceForCorporateContact = this.corporateContactList.some(
          corporateContact => {
            return (
              ...
              this.editDetails.contact.uniqueNumber !==
                corporateContact.uniqueNumber
            );
          }
        );
        if (this.emailExistenceForCorporateContact) {
          ...
          if (this.vendorForm.controls.email.valid) {
            const id = this.editDetails.contact
              ? this.editDetails.contact.id
              : 0;
            this.miscService
