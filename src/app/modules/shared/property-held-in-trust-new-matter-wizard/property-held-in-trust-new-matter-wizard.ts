...
@Component({
  selector: 'app-property-held-in-trust-new-matter-wizard',
  templateUrl: './property-held-in-trust-new-matter-wizard.html',
  styleUrls: ['./property-held-in-trust-new-matter-wizard.scss'],
  encapsulation: ViewEncapsulation.Emulated
})
export class PropertyHeldInTrustNewMatterWizardComponent implements OnInit {
  @Input() clientId;
  ...
  */
  ...
  toggleExpandRow(row) {
    if(this.selectedRow && this.selectedRow.trustNumber != row.trustNumber){
      this.table.rowDetail.collapseAllRows();
      removeAllBorders('app-property-held-in-trust-new-matter-wizard');
    }
  ...
  editPropertyHeldInTrust(item: any,$event = null) {
    ...
    this.propertyHeldInTrustItem = JSON.parse(JSON.stringify(item['propertyLineItems']));
  ...
  openInfo(content: any, className, winClass, isAdd = false,$event) {
    if ($event && $event.target) {
      $event.stopPropagation();
    }
    if (isAdd) {
  ...
  async submitAddPropertyHeldInTrustForm() {
    ...
    model.Id = this.selectedPropertyheldRows != null ? this.selectedPropertyheldRows['id'] : 0;
  ...
  saveNewLineItem() {
    ...
    if (!this.addPropertyHeldInTrustLineItemForm.valid) {
      if (this.addPropertyHeldInTrustLineItemForm.value['description'] == '' || this.addPropertyHeldInTrustLineItemForm.value['description'] == null) {
        this.descriptionError = true;
      }
      if (this.addPropertyHeldInTrustLineItemForm.value['value'] == '' || this.addPropertyHeldInTrustLineItemForm.value['value'] == null) {
  ...
  isPropertyLineItemAdded() {
    this.isPropertyHeldInTrustItemAdded = this.propertyHeldInTrustItem.length > 0;
  }
  ...
  get isAddTrustValid() {
    ...
    isValid = this.addPropertyHeldInTrustForm.valid && !this.addPropertyLineItem && this.propertyHeldInTrustItem.length && !isEditItemOpen;
    return isValid;
  }

  deletePropertyHeldRow(item: any,$event) {
    $event.stopPropagation();
  ...
  deletePropertyHeldInTrust() {
    if(this.propertyIdTodelete) {
      let index = this.propertyheldRows.findIndex(item => item.trustNumber === this.propertyIdTodelete);
  ...
  commonEmitToParent(propertyheldRows) {
    ...
    for (const data of this.propertyheldRows) {
      data['rowTotal'] = this.getRowTotal(data.trustNumber);
    }
