...
@Component({
  selector: 'app-add-blocked-employee-new-matter-wizard',
  templateUrl: './add-blocked-employee-new-matter-wizard.component.html',
  encapsulation: ViewEncapsulation.Emulated
})
export class AddBlockedEmployeeNewMatterWizardComponent implements OnInit {
  ...
  private getEmployeeList(): void {
    this.employeeService
      ...
      .subscribe(async (res: any) => {
        ...
        let result = response.results.filter((d: any) => {
          return this.alreadyBlockedEmployees.indexOf(d.id) == -1
        });

        let employees = result;
        this.employeesRows = employees.sort((a, b) =>
          a.lastName.localeCompare(b.lastName)
        );
        this.employeesRows.forEach(employee => {
          employee.fullName = employee.lastName ? employee.lastName + ', ' + employee.firstName : employee.firstName
        })
