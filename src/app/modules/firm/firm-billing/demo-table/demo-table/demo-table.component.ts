...
import { ConfirmDemoModelComponent } from '../create-demo-table/Confirm-demo-model/Confirm-demo-model.component';
...
@Component({
  selector: 'app-firm-demo-table',
  templateUrl: './demo-table.component.html',
  styleUrls: ['./demo-table.component.scss'],
  encapsulation: ViewEncapsulation.Emulated
})
export class FirmDemoTableComponent implements OnInit {
  ...
  public editRateTbl(row) {
    if (row && row.effectiveDate && moment(row.effectiveDate).isAfter(moment(new Date()))) {
      let modalRef = this.modalService.open(ConfirmDemoModelComponent, {
        centered: true,
        backdrop: 'static',
        keyboard: false,
      ...
      modalRef.result.then(res => {
        if (res === 'newchanges') {
          this.router.navigate(['/firm/edit-demo-table'], {queryParams: { rateTableId: row.id, newChanges : 'newchanges'}});
        } else if (res) {
          this.router.navigate(['/firm/edit-demo-table'], {queryParams: { rateTableId: row.id}});
        }
      });
    } else {
      this.router.navigate(['/firm/edit-demo-table'], {queryParams: { rateTableId: row.id}});
