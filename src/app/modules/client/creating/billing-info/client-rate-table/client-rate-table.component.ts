...
@Component({
  selector: 'app-client-rate-table',
  templateUrl: './client-rate-table.component.html',
  styleUrls: ['./client-rate-table.component.scss'],
  encapsulation: ViewEncapsulation.Emulated,
})
export class ClientRateTableComponent implements OnInit, AfterViewInit {
  ...
  public valueChange(event, row) {
    ...
    if (
      row.rateAmount &&
      +oldRow.rateAmount !== +(+row.rateAmount).toFixed(2)
    ) {
      row.isCustom = true;
  ...
  public rateFormat(row) {
    row.rateAmount = row.rateAmount
      ? (+row.rateAmount).toFixed(2)
      : row.rateAmount;
  }
